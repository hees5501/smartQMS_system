...
import { ProcedureData, ProcedureBasicInfoFormData } from '../types/ProcedureData'

interface ProcedureBasicInfoAreaProps {
  selectedProcedure: ProcedureData | null
  documentType?: DocumentType
}

export default function ProcedureBasicInfoArea({ selectedProcedure, documentType = 'procedure' }: ProcedureBasicInfoAreaProps) {
  const [formData, setFormData] = useState<ProcedureBasicInfoFormData>({
    구분: '절차서',
    분야: '',
    표준명: '',
    영문표준명: '',
  ...
  // 선택된 절차서가 변경될 때 폼 데이터 업데이트
  useEffect(() => {
    if (selectedProcedure) {
      setFormData({
        구분: selectedProcedure.구분,
        분야: selectedProcedure.분야,
        표준명: selectedProcedure.표준명,
        영문표준명: selectedProcedure.영문표준명,
        문서번호: selectedProcedure.문서번호,
        책임부서: selectedProcedure.책임부서,
        Rev: selectedProcedure.Rev,
        제정일자: selectedProcedure.제정일자,
        개정일자: selectedProcedure.개정일자,
        목적: '',
        적용범위: [],
        적용사업: '',
        프로세스관련번호: ''
  ...
  const handleInputChange = (field: keyof ProcedureBasicInfoFormData, value: string | string[] | number) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1rem' }}>
          ...
          <div>
            ...
            <input
              type="text"
              value={formData.영문표준명}
              onChange={(e) => handleInputChange('영문표준명', e.target.value)}
              disabled={!isEditing}
              className="input-field"
            />
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '1rem', marginBottom: '1rem' }}>
          ...
          <div>
            ...
            <input
              type="text"
              value={formData.책임부서}
              onChange={(e) => handleInputChange('책임부서', e.target.value)}
              disabled={!isEditing}
              className="input-field"
            />
          </div>
