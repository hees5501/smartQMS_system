...
import { GuideData, GuideBasicInfoFormData } from '../types/GuideData'

interface GuideBasicInfoAreaProps {
  selectedGuide: GuideData | null
  documentType?: DocumentType
}

export default function GuideBasicInfoArea({ selectedGuide, documentType = 'guide' }: GuideBasicInfoAreaProps) {
  const [formData, setFormData] = useState<GuideBasicInfoFormData>({
    구분: '지침서',
    분야: '',
    표준명: '',
    문서번호: '',
  ...
  // 선택된 지침서가 변경될 때 폼 데이터 업데이트
  useEffect(() => {
    if (selectedGuide) {
      setFormData({
        구분: selectedGuide.구분,
        분야: selectedGuide.분야,
        표준명: selectedGuide.표준명,
        문서번호: selectedGuide.문서번호,
        주관부서: '',
        Rev: selectedGuide.Rev,
        제정일자: selectedGuide.제정일자,
        개정일자: selectedGuide.개정일자,
        목적: '',
        적용범위: '',
        적용범위체크: [],
        적용사업: '',
  ...
  const handleInputChange = (field: keyof GuideBasicInfoFormData, value: string | string[] | number) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
